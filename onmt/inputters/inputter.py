...
def _build_fields_vocab(fields, counters, data_type, share_vocab,
    ...
    build_fv_args["sent1"] = dict(max_size=src_vocab_size, min_freq=src_words_min_frequency)
    build_fv_args["sent2"] = dict(max_size=src_vocab_size, min_freq=src_words_min_frequency)
    # build_fv_args["label"] = dict(max_size=src_vocab_size, min_freq=src_words_min_frequency)
    multifield = [fields["sent1"], fields["sent2"]]
    ...
    logger.info(" * merging fields vocab...")
    # src_field = src_multifield.base_field
    multifield = [fields["sent1"].base_field, fields["sent2"].base_field]
    _merge_field_vocabs(
        multifield, vocab_size=src_vocab_size,
        min_freq=src_words_min_frequency,
        vocab_size_multiple=vocab_size_multiple)
    logger.info(" * [sent1] merged vocab size: %d." % len(fields["sent1"].base_field.vocab))
    logger.info(" * [sent2] merged vocab size: %d." % len(fields["sent2"].base_field.vocab))
    # logger.info(" * [label] merged vocab size: %d." % len(fields["label"].base_field.vocab))
...
def _merge_field_vocabs(fields, vocab_size, min_freq,
                        vocab_size_multiple):
    # in the long run, shouldn't it be possible to do this by calling
    # build_vocab with both the src and tgt data?
    # specials = [f.unk_token for f in fields] + [f.]
    # specials = [tgt_field.unk_token, tgt_field.pad_token,
    #            tgt_field.init_token, tgt_field.eos_token]
    merged = sum(
        [f.vocab.freqs for f in fields], Counter()
        # [src_field.vocab.freqs, tgt_field.vocab.freqs], Counter()
    )
    merged_vocab = Vocab(
        merged, max_size=vocab_size, min_freq=min_freq
    )
    if vocab_size_multiple > 1:
        _pad_vocab_to_multiple(merged_vocab, vocab_size_multiple)
    for f in fields:
        f.vocab = merged_vocab
